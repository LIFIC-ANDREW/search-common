...
public class FileUtils {
    ...
    public static String getDirectoryPath(String filePath) {
        ...
        if (filePath != null) {
            ...
            if (idx != -1) path = filePath.substring(0, idx);
        }
        return path;
    }
    ...
    public static String getFileContent(String directory, String fileName, Charset charset) {
        try {
            return IOUtils.toString(new FileInputStream(
                    directory + ((directory.contains(File.separator)) ? File.separator : '/') + fileName)
                , charset);
        } catch (IOException e) {
            return null;
        }
    ...
    public static List<String> makeLinesToList(BufferedReader reader) throws IOException {
